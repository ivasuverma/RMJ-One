...
transaction so each calendar month always charges exactly 30 days' worth
of interest (see routers/gold_loans.py's _month_interest_daywise).
...
WHAT IT DOES
    ...
    already paid, so a closed loan can end up showing interest_paid
    different from the (recalculated) interest_due for some period —
    that's an expected, harmless artifact of correcting history after the
    fact, not a bug.
...
USAGE (on the server, from backend/ with the venv active)
    python migrations/recalc_gold_loan_interest_30day_months.py            # dry run
    python migrations/recalc_gold_loan_interest_30day_months.py --commit   # apply
"""
import asyncio
import os
import sys
...
def _day_balance(principal: float, principal_txns: list, d: date) -> float:
    balance = principal
