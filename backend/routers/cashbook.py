...
async def _notify_counter_over_limit(counter: dict, balance: float) -> None:
    ...
    default on both push and WhatsApp (cashbook_over_limit_employee, unlike
    every other cash_book alert, which are admin_only) — this is a standing
    safety rule tied to having access to the counter, not a discretionary
    ...
    async for e in db.employees.find(
        ...
        if wants_push or wants_wa:
            await notify_user(e['id'], 'Cash Limit Exceeded', body, '/cashbook', push=wants_push, whatsapp=wants_wa)
