...
async def get_karigar_ledger(kid: str, _: dict = Depends(require_ledger_access('karigar_ledger'))):
    ...
    entries = await db.karigar_ledger.find({'karigar_id': kid}, {'_id': 0}).sort('created_at', -1).to_list(1000)
    bal = _karigar_ledger_balances(entries).get(kid, {})
    cancelled = await db.karigar_ledger_voided.find({'karigar_id': kid}, {'_id': 0}).sort('voided_at', -1).to_list(200)
    return {
        'karigar': karigar, 'entries': entries, 'cancelled': cancelled,
        'weight_balance': round(bal.get('weight_bal', 0), 3), 'fine_weight_balance': round(bal.get('fine_bal', 0), 3),
        'amount_due': round(bal.get('amt_due', 0), 2),
    }
...
async def add_karigar_ledger_entry(kid: str, body: KarigarLedgerEntryIn, user=Depends(require_ledger_access('karigar_ledger'))):
    karigar = await db.karigars.find_one({'id': kid}, {'_id': 0})
    if not karigar: raise HTTPException(status_code=404, detail='Karigar not found')
    # Optional job attribution (Settle Balance flow) — no txn_id, unlike the
    # auto-generated issue/receive entries, which is what tells the delete
...
async def _void_karigar_entry(entry: dict, reason: str, user: dict) -> None:
    """Cancel an entry instead of deleting it: it (and its counter-entry on the metal ledger) moves to
    karigar_ledger_voided / metal_ledger_voided with who/when/why. Balances, the Loss Ledger, the Metal
    Ledger and every report read the live collections, so a cancelled entry stops counting everywhere at
    once — but it stays on record and is shown greyed out on the karigar's ledger."""
    iso = now_utc().isoformat()
    stamp = {'voided': True, 'void_reason': reason, 'voided_by': user['name'], 'voided_at': iso}
    await db.karigar_ledger_voided.insert_one({**entry, **stamp})
    async for m in db.metal_ledger.find({'source_entry_id': entry['id']}, {'_id': 0}):
        await db.metal_ledger_voided.insert_one({**m, **stamp})
    await db.metal_ledger.delete_many({'source_entry_id': entry['id']})
    await db.karigar_ledger.delete_one({'id': entry['id']})


async def _cancellable_entry(kid: str, entry_id: str) -> dict:
    entry = await db.karigar_ledger.find_one({'id': entry_id, 'karigar_id': kid}, {'_id': 0})
    if not entry:
        raise HTTPException(status_code=404, detail='Ledger entry not found')
    # txn_id marks an entry auto-generated by a repair/sample issue or receive — those are undone from that
    # tag's history. A manual Settle Balance entry (and a loss the shop absorbed) can be cancelled here.
    if entry.get('txn_id') and not entry.get('absorbs'):
        raise HTTPException(status_code=400, detail="This entry is tied to a repair tag or sample — undo the issue/receive from its history instead")
    return entry


class CancelEntryIn(BaseModel):
    reason: str
...
def _need_reason(reason: str) -> str:
    r = (reason or '').strip()
    if len(r) < 3:
        raise HTTPException(status_code=400, detail='Give a reason (a few words is enough)')
    return r[:200]


@router.post('/karigars/{kid}/ledger/{entry_id}/cancel')
async def cancel_karigar_ledger_entry(kid: str, entry_id: str, body: CancelEntryIn, user=Depends(require_ledger_edit_right('karigar_ledger', 'delete'))):
    reason = _need_reason(body.reason)
    entry = await _cancellable_entry(kid, entry_id)
    await _void_karigar_entry(entry, reason, user)
    await log_audit(user, 'karigar_ledger.cancel', 'karigar', kid, '', {'entry_id': entry_id, 'type': entry.get('type'), 'reason': reason})
    return {'ok': True}
...
async def delete_karigar_ledger_entry(kid: str, entry_id: str, user=Depends(require_ledger_edit_right('karigar_ledger', 'delete'))):
    # Kept for older clients: it no longer erases anything — the entry is cancelled and stays on record.
    entry = await _cancellable_entry(kid, entry_id)
    await _void_karigar_entry(entry, 'Deleted (no reason given)', user)
    await log_audit(user, 'karigar_ledger.cancel', 'karigar', kid, '', {'entry_id': entry_id, 'type': entry.get('type'), 'reason': 'Deleted (no reason given)'})
    return {'ok': True}


class EditKarigarEntryIn(BaseModel):
    weight: Optional[float] = None    # gold entries, fine grams as entered
    amount: Optional[float] = None    # money entries
    note: Optional[str] = None
    reason: str


@router.put('/karigars/{kid}/ledger/{entry_id}')
async def edit_karigar_ledger_entry(kid: str, entry_id: str, body: EditKarigarEntryIn, user=Depends(require_ledger_edit_right('karigar_ledger', 'edit'))):
    """Correct an entry the safe way: post a corrected replacement (same date, same job) and cancel the
    old one with the reason. Nothing is overwritten — both stay visible."""
    reason = _need_reason(body.reason)
    entry = await _cancellable_entry(kid, entry_id)
    new = {**entry, 'id': str(uuid.uuid4()), 'replaces': entry['id'], 'edited_by': user['name'], 'edit_reason': reason,
           'edited_at': now_utc().isoformat()}
    if body.note is not None:
        new['note'] = body.note
    if entry['type'] in ('gold_out', 'gold_in', 'loss'):
        w = abs(body.weight) if body.weight is not None else entry.get('weight')
        if not w or w <= 0:
            raise HTTPException(status_code=400, detail='Enter a gold weight greater than 0')
        ratio = (entry.get('fine_weight') / entry['weight']) if entry.get('fine_weight') is not None and entry.get('weight') else 1
        new['weight'] = round(w, 3)
        new['fine_weight'] = round(w * ratio, 3)
        await post_gold_ledger_entry(new)
    else:
        a = body.amount if body.amount is not None else entry.get('amount')
        if not a:
            raise HTTPException(status_code=400, detail='Enter an amount')
        new['amount'] = a if entry['type'] == 'adjustment' else abs(a)   # an adjustment carries its own sign
        await db.karigar_ledger.insert_one(dict(new))
    await _void_karigar_entry(entry, f'Edited — {reason}', user)
    await log_audit(user, 'karigar_ledger.edit', 'karigar', kid, '', {'entry_id': entry_id, 'replacement': new['id'], 'reason': reason})
...
def _escpos_size_for_pt(pt: int) -> bytes:
    """Maps a Print Master point size to an ESC/POS GS ! byte. Scales height
    only (width multiplier stays 0/1x) so the fixed 42-char-per-line wrap
    math above stays valid no matter what size is chosen — widening
    characters too would overflow the 80mm paper instead of wrapping."""
    mult = max(0, min(7, round((pt - 10) / 3)))
    return _GS + b'!' + bytes([mult])
