...
// The Cash Book: one running statement per location and day (Received / Paid / Transfer in a single ordered list, with
// the balance after every entry), a summary card on top, and one ＋ button that opens the compose sheet.
// The previous two-column layout is still available from the header ("Classic view").
  ...
  return (
    <SafeAreaView style={styles.root} edges={['top']} testID="cashbook-screen">
      <View style={styles.header}>
        ...
        <Pressable onPress={() => router.push('/cashbook/classic' as any)} style={styles.classicBtn} testID="cashbook-classic-btn" hitSlop={8}>
          <Ionicons name="reader-outline" size={15} color={colors.onSurfaceSecondary} />
          <Text style={styles.classicText}>Classic view</Text>
        </Pressable>
        {isOwner ? (
          <Pressable onPress={() => router.push('/cashbook/classic?manage=1' as any)} style={styles.iconBtn} testID="cashbook-settings-btn" hitSlop={12}>
