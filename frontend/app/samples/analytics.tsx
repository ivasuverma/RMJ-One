...
// Owner-only Stock In/Out dashboard — same day/week/month shape as Cash
// Book Analytics: how many pieces (and how much gold) went out and came
// back, and where it's split by issue type and by karigar.
...
export default function SamplesAnalyticsScreen() {
  ...
  // straight back to the regular Stock In/Out list instead of surfacing an
  // error, since owner/admin/accountant/employee all reach this same route
  // via the Dashboard's Stock In/Out tile.
  useFocusEffect(useCallback(() => {
    if (user && user.role !== 'owner') router.replace('/samples?status=with_karigar');
  }, [user, router]));

  const load = useCallback(async () => {
    setLoading(true);
    try { setData(await api.get<Analytics>(`/samples/analytics?period=${period}&date=${date}`)); }
        ...
        <ScrollView contentContainerStyle={{ padding: spacing.lg, paddingBottom: 60 }}>
          <View style={styles.summaryRow}>
            <View style={styles.summaryCard}>
              ...
              <Text style={styles.summaryValue}>{data.total_issued}</Text>
            </View>
            <View style={styles.summaryCard}>
            ...
            <View style={styles.summaryCard}>
              <Text style={styles.summaryLabel}>Net Out</Text>
              <Text style={[styles.summaryValue, { color: netWeight > 0 ? colors.onWarning : colors.onSuccess }]}>{netWeight.toFixed(3)}g</Text>
            </View>
          </View>
            ...
            <>
              ...
              <View style={styles.trendCard}>
                <View style={styles.legendRow}>
                  <View style={styles.legendItem}><View style={[styles.legendDot, { backgroundColor: colors.brandSecondary }]} /><Text style={styles.legendText}>Issued</Text></View>
                      ...
                      return (
                        <View key={t.date} style={styles.barCol}>
                          <View style={styles.barPair}>
                            <View style={[styles.bar, { height: Math.max(2, (t.issued / maxTrend) * 90), backgroundColor: colors.brandSecondary }]} />
          ...
          <Text style={styles.sectionLabel}>Issued by Type</Text>
          <TypeBreakdown data={data.by_type} total={totalByType} colors={colors} styles={styles} />

          {data.top_karigars.length > 0 && (
            <>
              <Text style={styles.sectionLabel}>Top Karigars (Issued)</Text>
...
function TypeBreakdown({ data, total, colors, styles }: { data: TypeCount[]; total: number; colors: ThemeColors; styles: any }) {
  if (data.length === 0 || total <= 0) {
    return <Text style={styles.empty}>No samples issued this period.</Text>;
  }
  const size = 140;
  const stroke = 24;
