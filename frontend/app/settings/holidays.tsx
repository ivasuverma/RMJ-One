...
export default function HolidaysScreen() {
  ...
  const remove = (h: H) => {
    Alert.alert('Delete holiday', `Remove ${h.name}?`, [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => { try { await api.del(`/holidays/${h.id}`); await load(); } catch (_e) {} } },
    ]);
  };
    ...
    <SafeAreaView style={styles.root} edges={['top']} testID="holidays-screen">
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        ...
        >
          ...
          <Text style={[styles.section, { marginTop: spacing.xl }]}>Upcoming</Text>
          {items.map((h) => (
