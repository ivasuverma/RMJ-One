...
type ModuleDef = { key: string; label: string; default_roles: string[] };
type Account = {
...
export default function UserRolesScreen() {
  ...
  const save = async (acc: Account) => {
    setSavingId(acc.id);
    try {
      await api.put(`/access/accounts/${acc.id}`, { module_access: Array.from(selection[acc.id] || []) });
      await load();
      setExpandedId(null);
  ...
  const resetToDefault = async (acc: Account) => {
    setSavingId(acc.id);
    try {
      await api.put(`/access/accounts/${acc.id}`, { module_access: null });
      await load();
      setExpandedId(null);
    } catch (e: any) { Alert.alert('Failed', e?.detail || 'Could not save. Please try again.'); }
    finally { setSavingId(null); }
      ...
      {loading ? (
        ...
        >
          <Text style={styles.hint}>
            ...
            them here — a custom list always overrides the default, even if you uncheck everything.
          </Text>
                ...
                {isExpanded && (
                  <View style={styles.expandBody}>
                    {modules.map((m) => {
                      ...
                      return (
                        <Pressable key={m.key} onPress={() => toggleModule(acc.id, m.key)} style={styles.modRow} testID={`mod-${acc.id}-${m.key}`}>
