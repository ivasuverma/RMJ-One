...
export default function CashBookScreen() {
  ...
  // Opened from the new Cash Book's gear button: go straight to counter management.
  useEffect(() => { if (manage === '1' && isOwner) { setCounterForm(null); setMode('settings'); } }, [manage, isOwner]);
  ...
  return (
    <SafeAreaView style={styles.root} edges={['top']} testID="cashbook-screen">
      <View style={styles.header}>
        ...
        {mode === 'view' ? (
          <Pressable onPress={() => router.replace('/cashbook' as any)} style={styles.iconBtn} testID="cashbook-new-view-btn" hitSlop={12} accessibilityLabel="Switch to the new Cash Book view">
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
          <ScrollView contentContainerStyle={{ padding: spacing.lg, paddingBottom: 60 }} keyboardShouldPersistTaps="handled">
            ...
            {!editing && transferOptions.length > 1 && (
              <>
                <Pressable
                  onPress={() => { setIsTransfer((v) => !v); setTransferCounterId(''); }}
            ...
            <View style={styles.amountNameRow}>
...
const makeStyles = (colors: ThemeColors) => StyleSheet.create({
  ...
  transferToggleBtn: {
    ...
    borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surfaceSecondary, marginBottom: spacing.sm,
  },
