...
export type RepairItemStatus = 'received' | 'with_karigar' | 'ready' | 'pending_delivery' | 'delivered';
...
export function repairStatusColors(status: RepairItemStatus, colors: ThemeColors): { bg: string; fg: string; border: string } {
  switch (status) {
    ...
    case 'pending_delivery': return { bg: colors.warning, fg: colors.onWarning, border: colors.warning };
